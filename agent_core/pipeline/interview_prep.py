...
async def predict_questions(job, config, llm_provider, direction=None):
    if direction is None:
        direction = getattr(job, 'direction', '') or (list(config.directions.keys())[0] if config.directions else '')  # noqa: E501
    try:
        resume = load_resume(config, direction)[:3000]
    except Exception:
        resume = f"Candidate for {job.title}"
    prompt = PREDICT_PROMPT.format(title=job.title, company=job.company,
        location=job.location or config.search_location,
        description=job.description[:3000], resume=resume)
    r = await llm_provider.chat(messages=[{"role":"user","content":prompt}], temperature=0.6, max_tokens=config.llm.max_tokens)  # noqa: E501
    t = r.strip()
    if "```json" in t:
...
def mock_interview(job, config, llm_provider, direction=None):
    ...
    try:
        ...
        resume = f"Candidate for {job.title}"
    msgs = [{"role":"system","content":MOCK_SYSTEM.format(company=job.company,title=job.title,resume_summary=resume,jd_summary=job.description[:1000])}]  # noqa: E501
    transcript = [f"模拟面试记录\n{job.title} @ {job.company}\n{'='*50}\n"]
