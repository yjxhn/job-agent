...
def add_application(db, job_id, resume_version="", notes="") -> int:
    ...
    dup = db.execute(
        "SELECT id FROM applications WHERE job_id=? AND status='已投递'",
        (job_id,)).fetchone()
    if dup:
...
def list_applications(db, status_filter=None) -> list[dict]:
    ...
    if status_filter:
        rows = db.execute("SELECT * FROM applications WHERE status=? ORDER BY updated_at DESC",
...
def _enrich(app, db):
    try:
        job = db.execute("SELECT title,company,location,urls FROM jobs WHERE id=?",
                        (app["job_id"],)).fetchone()
        if job:
...
def _add_timeline(db, app_id, frm, to, ts):
    try:
        db.execute("INSERT INTO timelines (application_id,from_status,to_status,created_at) "
                   "VALUES (?,?,?,?)", (app_id, frm, to, ts))
        db.commit()
    except Exception as e:
...
def get_timeline(db, app_id) -> list[dict]:
    ...
    rows = db.execute(
        "SELECT * FROM timelines WHERE application_id=? ORDER BY created_at ASC",
        (app_id,)).fetchall()
