...
def test_offer_eval_parses_json(cfg):
    ...
    r = asyncio.run(
        evaluate(cfg, provider, company="某公司", title="AMR", salary="15K", bonus="2个月")
    )
...
def test_salary_advice_parses_json(cfg):
    ...
    resp = (
        '{"anchor":"18K","leverage":["x"],"concessions":["y"],"scripts":["z"],"confidence":"high"}'
    )
    provider = FakeProvider([resp])
...
def test_run_scheduled_search_skips_when_disabled(cfg, db, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(S, "STATE_FILE", tmp_path / "state.json")
    S._save({"enabled": False, "last_run": None, "runs": 0, "directions": [], "last_error": None})
    asyncio.run(S.run_scheduled_search(cfg, FakeProvider([]), db))
...
def test_enrich_boss_success(monkeypatch, cfg):
    ...
    monkeypatch.setattr(
        "agent_core.platforms.boss_zhipin.BossZhipinAdapter.fetch_full_jd", fake_fetch
    )
    monkeypatch.setattr(
        "agent_core.platforms.boss_zhipin.BossZhipinAdapter.__init__", lambda self: None
    )
...
def test_enrich_liepin_success(monkeypatch, cfg):
    ...
    monkeypatch.setattr("agent_core.platforms.liepin.LiepinAdapter.fetch_full_jd", fake_fetch)
    monkeypatch.setattr("agent_core.platforms.liepin.LiepinAdapter.__init__", lambda self: None)
...
def test_tailor_open_job_link(monkeypatch):
    ...
    job = _job(platforms=["boss_zhipin"], urls={"boss_zhipin": "https://zhipin.com/job/123"})
    open_job_link(job)
...
def test_search_one_general_exception(monkeypatch, cfg):
    ...
    async def raising_adapter_search(
        self, keywords, location, cookie_path=None, headless=False, rate_limit_seconds=None
    ):
