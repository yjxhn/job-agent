...
使用场景：Boss直聘等强反爬站点，Playwright 自动登录被拦截。改为：
  ...
  4. 运行: python scripts/import_cookies.py <export.json> <platform> [--domain zhipin.com]
...
# Add project root to sys.path so `agent_core` is importable no matter where
# the script lives (scripts/ is one level below the project root).
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, PROJECT_ROOT)

from agent_core.platforms.cookie_utils import convert_and_save  # noqa: E402


def main():
    if len(sys.argv) < 3:
        print(
            "用法: python scripts/import_cookies.py <export.json> <platform> "
            "[--domain zhipin.com]"
        )
        print(
            "示例: python scripts/import_cookies.py data/cookies/boss_export.json "
            "boss --domain zhipin.com"
        )
        sys.exit(1)
    ...
    try:
        ...
        print(
            f"     session cookies found: "
            f"{result['session_found'] if result['session_found'] else '(none)'}"
        )
